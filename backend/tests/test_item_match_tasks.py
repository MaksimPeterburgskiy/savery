...
def test_create_item_match_candidates_generates_unique_candidates() -> None:
    ...
    try:
        with session_scope() as session:
            ...
            session.flush()
            entity_ids[Product].extend([product_milk.id, product_oatmilk.id, product_irrelevant.id])
        ...
        with session_scope() as session:
            ...
            entity_ids[ItemMatchCandidate].extend(cid for cid in candidate_ids if cid not in entity_ids[ItemMatchCandidate])
...
def test_create_item_match_candidates_handles_branded_and_unbranded_items() -> None:
    ...
    try:
        with session_scope() as session:
            ...
            session.add_all([list_item_cream, list_item_brand, list_item_green, list_item_org_green])
            session.flush()
        ...
        with session_scope() as session:
            ...
            for item_id, expected_names in list_item_ids.items():
                ...
                observed_names = {session.get(Product, candidate.product_id).name for candidate in by_item[item_id]}
...
def test_fanout_candidates_to_item_matches_creates_matches_for_selected_stores() -> None:
    ...
    try:
        with session_scope() as session:
            ...
            session.flush()
            entity_ids[ItemMatchCandidate].extend([candidate_milk.id, candidate_oatmilk.id, candidate_rejected.id])
        ...
        with session_scope() as session:
            ...
            entity_ids[ItemMatch].extend(mid for mid in match_ids if mid not in entity_ids[ItemMatch])
...
def test_fanout_candidates_handles_branded_and_store_specific_products() -> None:
    ...
    try:
        with session_scope() as session:
            ...
            session.add_all([product_philly, product_store_generic, product_other_store_generic])
            session.flush()
            entity_ids[Product].extend([product_philly.id, product_store_generic.id, product_other_store_generic.id])
            ...
            session.flush()
            entity_ids[StoreProduct].extend(sp.id for sp in philly_store_products + [hannaford_generic_sp, pricechopper_generic_sp])
            ...
            session.add_all([candidate_philly, candidate_hannaford_generic, candidate_pricechopper_generic])
            session.flush()
            entity_ids[ItemMatchCandidate].extend([candidate_philly.id, candidate_hannaford_generic.id, candidate_pricechopper_generic.id])
        ...
        with session_scope() as session:
            ...
            assert by_candidate[candidate_pricechopper_generic_id][0].store_id == pricechopper_id
    finally:
        _delete_entities(entity_ids)

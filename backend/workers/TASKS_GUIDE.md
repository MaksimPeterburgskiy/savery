...
3. **Record progress** inside the task using `mark_job_running`, `record_job_progress`, `mark_job_success`, and `mark_job_failed` to persist status in the database. These helpers now require `task=self` (and optional `include_status=True` for the full payload) so they always invoke `self.update_state(...)` in lockstep with DB writes.
